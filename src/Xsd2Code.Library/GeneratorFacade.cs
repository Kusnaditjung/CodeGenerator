...
namespace Xsd2Code.Library
{
    ...
    public class GeneratorFacade
    {
        ...
        {
            bool isMultiple = GeneratorContext.GeneratorParams.IsMultipleFiles;            
            ...
            #endregion
            Result<CodeNamespace> result;
            try
            {
                try
                {
                    result = Generator.Process(GeneratorContext.GeneratorParams);
                    if (!result.Success) return result;
                    
                    WriteTempOutputFile(outputFilePath, isMultiple, result.Entity);
                }
                catch (Exception e)
                {
                ...
                if (isMultiple)
                {
                    foreach (CodeTypeDeclaration codeType in result.Entity.Types)
                    {
                        string outputFileName = $"{outputFilePath}\\{codeType.Name}.cs";
                        ...
                        if (outputFile.Exists)
                        {
                            ...
                            {
                                var errorMessage = "Failed to generate code\n";
                                errorMessage += outputFileName + " is write protect";
                                return new Result(false, MessageType.Error, errorMessage);
                            }
                        }

                        GenerateOutputFileWithAutoGenText(outputFileName);
                    }
                }
                else
                {
                ...
                try
                {
                    DeleteTempOutputFile(outputFilePath, isMultiple, result);
                }
                catch (Exception ex)
                {
        ...
        private static void GenerateOutputFileWithAutoGenText(string outputFileName)
        {
            using (var outputStream = new StreamWriter(outputFileName, false))
            {
                ...
                using (TextReader streamReader = new StreamReader(outputFileName + ".tmp"))
                {
                    string line;
        ...
        private void WriteTempOutputFile(string outputFilePath, bool isMultiple, CodeNamespace ns)
        {
            if (isMultiple)
            {
                CodeNamespace copiedNamespace = CopiedTypesOnly(ns);

                foreach (CodeTypeDeclaration codeType in copiedNamespace.Types)
                {
                    ns.Types.Clear();
                    ns.Types.Add(codeType);

                    using (var outputStream = new StreamWriter($"{outputFilePath}\\{codeType.Name}.cs.tmp", false))
                    {
                        this.providerField.GenerateCodeFromNamespace(ns, outputStream, new CodeGeneratorOptions());
                    }
                }

                CopyBackTypes(ns, copiedNamespace);
            }
            else
            {
        ...
        private static void CopyBackTypes(CodeNamespace ns, CodeNamespace copiedNamespace)
        {
            foreach (CodeTypeDeclaration ctd in copiedNamespace.Types)
            {
                ns.Types.Add(ctd);
            }
        }

        private static CodeNamespace CopiedTypesOnly(CodeNamespace ns)
        {
            CodeNamespace copiedNamespace = new CodeNamespace();

            foreach (CodeTypeDeclaration codeType in ns.Types)
            {
                copiedNamespace.Types.Add(codeType);
            }

            return copiedNamespace;
        }

        private static void DeleteTempOutputFile(string outputFilePath, bool isMultiple, Result<CodeNamespace> result)
        {            
            if (isMultiple)
            {
                foreach (CodeTypeDeclaration codeType in result.Entity.Types)                                    
                    new FileInfo($"{outputFilePath}\\{codeType.Name}.cs.tmp").Delete();                    
            }
            else
            {                
                new FileInfo($"{outputFilePath}.tmp").Delete();
            }
        }
